...
class CommandExecutorHandler():
    ...
    def execute_command(self, request, results):
        ...
        try:
            ...
            results.append(e)
            return payload_result.update(utils.build_ret_data(False, err_msg))

        # deactivate_venv(blueprint_id)
        #Since return code is only used to check if it's zero (success), we can just return success flag instead.
        payload_result["cds_is_successful"] = (rc == 0)
        ###TODO: debug what is in part.get_payload(), and whether it can be wrapped in build_ret_data
        return payload_result.update(utils.build_ret_data(True, ""))
