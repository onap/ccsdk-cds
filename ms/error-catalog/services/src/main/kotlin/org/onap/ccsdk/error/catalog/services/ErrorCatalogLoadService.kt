...
open class ErrorCatalogLoadDBService(
    ...
    init {
        val errorMessagesForBP = arrayListOf<ErrorMessageModel>()
        val errorMessagesForSDCListener = arrayListOf<ErrorMessageModel>()
        val errorMessagesForSelfServiceApi = arrayListOf<ErrorMessageModel>()
        errorMessagesForBP.add(ErrorMessageModel("GENERIC_FAILURE",
                "Internal error in Blueprint Processor run time", "Contact CDS administrator team."))
        errorMessagesForSDCListener.add(ErrorMessageModel("GENERIC_FAILURE",
                "Internal error in SDC Listener", "Contact CDS administrator team."))
        errorMessagesForSelfServiceApi.add(ErrorMessageModel("GENERIC_PROCESS_FAILURE",
                "Internal error while processing REST call to the Self Service API.",
                "Verify the request and try again."))
        errorMessagesForSelfServiceApi.add(ErrorMessageModel("INVALID_FILE_EXTENSION",
                "Failed trying to upload a non ZIP file format",
                "Please reload your file and make sure it is in ZIP format."))
        errorCatalogDBService.saveDomain("org.onap.ccsdk.cds.blueprintsprocessor", "cds",
                "Blueprint run time processor domain", errorMessagesForBP)
        errorCatalogDBService.saveDomain("org.onap.ccsdk.cds.blueprintsprocessor.functions.python.executor",
                "cds", "Remote python executor for BP domain", errorMessagesForBP)
        errorCatalogDBService.saveDomain("org.onap.ccsdk.cds.sdclistener", "cds",
                "SDC Listener domain", errorMessagesForSDCListener)
        errorCatalogDBService.saveDomain("org.onap.ccsdk.cds.blueprintsprocessor.selfservice.api",
                "cds", "Self Service API domain in BP", errorMessagesForSelfServiceApi)
    }
