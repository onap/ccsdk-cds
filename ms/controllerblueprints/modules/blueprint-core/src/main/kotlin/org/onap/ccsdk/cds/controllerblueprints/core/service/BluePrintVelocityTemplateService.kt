...
object BluePrintVelocityTemplateService: BlueprintTemplateService {
    ...
    override fun generateContentFromFiles(templatePath: String, jsonPath: String, ignoreJsonNull: Boolean,
                                          additionalContext: MutableMap<String, Any>): String {

        val json = JacksonUtils.getContent(jsonPath)
        val template = JacksonUtils.getContent(templatePath)

        return BluePrintVelocityTemplateService.generateContent(template, json, ignoreJsonNull, additionalContext)
    }
}
