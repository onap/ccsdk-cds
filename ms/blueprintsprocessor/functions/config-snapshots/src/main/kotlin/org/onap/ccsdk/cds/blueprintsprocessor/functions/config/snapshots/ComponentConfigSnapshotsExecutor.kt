 ...
 *
 * Component that retrieves the saved configuration snapshot as identified by the input parameters, named
 * resource-id and resource-type.
 *
 * It reports the content of the requested snapshot via properties, config-snapshot-status and config-snapshot-value
 * In case of error, details can be found in the config-snapshot-status and config-snapshot-message properties
 *
 * @author Serge Simard
 */
...
open class ComponentConfigSnapshotsExecutor(private val cfgSnapshotService: ResourceConfigSnapshotService) :
    ...
    companion object {
        ...
        const val RESPONSE_STATUS = "config-snapshot-status"
        const val RESPONSE_MESSAGE = "config-snapshot-message"
        const val RESPONSE_SNAPSHOT = "config-snapshot-value"
    ...
    override suspend fun processNB(executionRequest: ExecutionServiceInput) {
        ...
        when (operation) {
            ...
            else -> setNodeOutputErrors(RESPONSE_STATUS_ERROR,
    ...
    override suspend fun recoverNB(runtimeException: RuntimeException, executionRequest: ExecutionServiceInput) {
        setNodeOutputErrors(RESPONSE_STATUS_ERROR, "Error : ${runtimeException.message}")
    }
    ...
    private suspend fun fetchConfigurationSnapshot(resourceId: String, resourceType: String,
        ...
        try {
            ...
            setNodeOutputProperties(RESPONSE_STATUS_SUCCESS, cfgSnapshotValue)
            ...
            setNodeOutputErrors(RESPONSE_STATUS_ERROR, message)
        }
    }
    ...
    private suspend fun storeConfigurationSnapshot(cfgSnapshotValue : String, resourceId: String, resourceType: String,
        ...
        if (cfgSnapshotValue.isNotEmpty()) {
            ...
            setNodeOutputProperties(RESPONSE_STATUS_SUCCESS, cfgSnapshotSaved.config_snapshot ?: "" )
        } else {
    ...
    private suspend fun compareConfigurationSnapshot(resourceId: String, resourceType: String, contentType : String) {
        ...
        when (contentType.toUpperCase()) {
            DIFF_JSON -> {
                ...
                setNodeOutputProperties(RESPONSE_STATUS_SUCCESS, patchNode.toString())
            }
            DIFF_XML -> {
                val cfgRunning = cfgSnapshotService.findByResourceIdAndResourceTypeAndStatus(resourceId, resourceType, RUNNING)
                val cfgCandidate = cfgSnapshotService.findByResourceIdAndResourceTypeAndStatus(resourceId, resourceType, CANDIDATE)
                ...
                setNodeOutputProperties(RESPONSE_STATUS_SUCCESS, formatXmlDifferences(myDiff))
            }
            else -> {
    ...
    private fun setNodeOutputProperties(status: String, snapshot: String) {
        setAttribute(RESPONSE_STATUS, status.asJsonPrimitive())
        setAttribute(RESPONSE_SNAPSHOT, snapshot.asJsonPrimitive())
        log.info("Setting output $RESPONSE_STATUS=$status, $RESPONSE_SNAPSHOT=$snapshot ")
    }
    ...
    private fun setNodeOutputErrors(status: String, message: String) {
        setAttribute(RESPONSE_STATUS, status.asJsonPrimitive())
        setAttribute(RESPONSE_MESSAGE, message.asJsonPrimitive())
        setAttribute(RESPONSE_SNAPSHOT, "".asJsonPrimitive())

        log.info("Setting error and output $RESPONSE_STATUS=$status, $RESPONSE_MESSAGE=$message ")

        addError(status, RESPONSE_MESSAGE, message)
    }
