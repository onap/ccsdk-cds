 ...
 */
@Service
class ResourceConfigSnapshotService(private val cfgSnapshotRepository: ResourceConfigSnapshotRepository) {
    ...
    suspend fun findByResourceIdAndResourceTypeAndStatus(resourceId: String, resourceType: String,
            ...
            withContext(Dispatchers.IO) {
                cfgSnapshotRepository.findByResourceIdAndResourceTypeAndStatus(resourceId, resourceType, status)
    ...
    suspend fun write(snapshot: String, resId: String, resType: String,
            ...
            withContext(Dispatchers.IO) {
                ...
                if (resId.isNotEmpty() && resType.isNotEmpty()) {
                    cfgSnapshotRepository.findByResourceIdAndResourceTypeAndStatus(resId, resType, status)?.let {
                        log.info("Overwriting configuration snapshot entry for resourceId=($resId), resourceType=($resType), status=($status)")
                        cfgSnapshotRepository.deleteByResourceIdAndResourceTypeAndStatus(resId, resType, status)
                    }
                }
                var storedSnapshot: ResourceConfigSnapshot
                try {
                    storedSnapshot = cfgSnapshotRepository.saveAndFlush(resourceConfigSnapshotEntry)
                    log.info("Stored configuration snapshot for resourceId=($resId), resourceType=($resType), " +
                            "status=($status), dated=(${storedSnapshot.createdDate})")
