...
open class MessageOutputProcessor(private val messagePrioritizationService: MessagePrioritizationService)
    ...
    override suspend fun processNB(key: String, value: MessagePrioritization) {
        messagePrioritizationService.updateMessageState(value.id, MessageState.EXPIRED.name)
        processorContext.forward(value.id, value, To.child(MessagePrioritizationConstants.SINK_OUTPUT))
    }
}