...
interface MessagePrioritizationStateService {
    ...
    suspend fun getCorrelatedMessages(group: String, types: List<String>?, correlationIds: String): List<MessagePrioritization>?
...
open class MessagePrioritizationStateServiceImpl(
    ...
    override suspend fun getCorrelatedMessages(group: String, types: List<String>?, correlationIds: String)
            : List<MessagePrioritization>? {
        return if (!types.isNullOrEmpty()) {
            prioritizationMessageRepository.findByGroupAndTypesAndCorrelationId(group, types, correlationIds)
        } else {
            prioritizationMessageRepository.findByGroupAndCorrelationId(group, correlationIds)
        }
    }
