...
class NetconfSessionImpl(private val deviceInfo: DeviceInfo, private val rpcService: NetconfRpcService,
    ...
    private fun exchangeHelloMessage() {
        ...
        val capabilitiesFoundSB = StringBuilder()
        ...
        while (capabilityMatcher.find()) {
            ...
            capabilitiesFoundSB.append((foundCapability + "\n"))
            numCapabilities++
        }
        log.info("Found $numCapabilities for device $deviceInfo: \n" + capabilitiesFoundSB.toString())
    }

    private fun setupHandler() {
        streamHandler = NetconfDeviceCommunicator(
            sshClientWrapper.getClientChannelInputStream(),
            sshClientWrapper.getClientChannelOutputStream(),
            deviceInfo,
