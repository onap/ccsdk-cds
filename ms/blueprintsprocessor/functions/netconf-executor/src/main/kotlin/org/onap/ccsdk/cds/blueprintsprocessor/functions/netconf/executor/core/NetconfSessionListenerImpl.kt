...
internal class NetconfSessionListenerImpl(private val session: NetconfSessionImpl) : NetconfSessionListener {
    override fun notify(event: NetconfReceivedEvent) {
        when (event.getType()) {
            ...
            NetconfReceivedEvent.Type.DEVICE_ERROR -> session.addDeviceErrorReply(event.getMessagePayload())
            NetconfReceivedEvent.Type.DEVICE_REPLY -> session.addDeviceReply(event.getMessageID(), event.getMessagePayload())
        }
    }
}