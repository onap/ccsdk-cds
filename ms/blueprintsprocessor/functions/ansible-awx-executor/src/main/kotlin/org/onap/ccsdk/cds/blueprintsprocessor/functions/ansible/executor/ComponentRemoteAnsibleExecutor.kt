...
open class ComponentRemoteAnsibleExecutor(private val blueprintRestLibPropertyService: BluePrintRestLibPropertyService)
    ...
    override suspend fun processNB(executionRequest: ExecutionServiceInput) {
        ...
        val restClientService = blueprintRestLibPropertyService.blueprintWebClientService(endpointSelector)

        log.info("Processing with endpoint $endpointSelector for inputs $operationInputs")
        ...
        try {
            ...
            if (jtId.isNotEmpty()) {
                ...
                addError("error", ATTRIBUTE_EXEC_CMD_LOG, "ERROR : job template ${jobTemplateName} does not exists")
            }
        } catch (e: Exception) {
            addError("error", ATTRIBUTE_EXEC_CMD_STATUS, "Failed")
    ...
    private fun runJobTemplateOnAWX(awxClient : BlueprintWebClientService, job_template_name: String?, jtId: String) {
        ...
        if (response.status in HTTP_SUCCESS) {
            ...
            setNodeOutputProperties( response.status.asJsonPrimitive(), message.asJsonPrimitive())
        }
    }
