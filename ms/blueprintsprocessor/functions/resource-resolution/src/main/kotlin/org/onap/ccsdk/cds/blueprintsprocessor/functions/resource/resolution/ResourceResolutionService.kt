...
open class ResourceResolutionServiceImpl(private var applicationContext: ApplicationContext,
    ...
    override suspend fun resolveResources(bluePrintRuntimeService: BluePrintRuntimeService<*>, nodeTemplateName: String,
        ...
        if (existingResourceResolution.isNotEmpty()) {
            markSuccessAlreadyResolvedAssignment(existingResourceResolution, resourceAssignments)
        }
    ...
    // Check whether resolution already exist in the database with for the specified resolution-key or resourceId/resourceType
    private suspend fun isNewResolution(bluePrintRuntimeService: BluePrintRuntimeService<*>, properties: Map<String, Any>, artifactPrefix: String): List<ResourceResolution> {
        ...
        if (resolutionKey.isNotEmpty()) {
            val existingResourceAssignments = resourceResolutionDBService.findByResolutionKeyAndOccurrence(bluePrintRuntimeService, resolutionKey, occurrence, artifactPrefix)
            ...
            val existingResourceAssignments =
                resourceResolutionDBService.findByResourceIdAndResourceType(bluePrintRuntimeService, resourceId, resourceType, occurrence, artifactPrefix)
            if (existingResourceAssignments.isNotEmpty()) {
    ...
    // Triage the remaining resource assignment to execute, by filtering out the ones with success status
    private fun markSuccessAlreadyResolvedAssignment(resourceResolutionList: List<ResourceResolution>,
