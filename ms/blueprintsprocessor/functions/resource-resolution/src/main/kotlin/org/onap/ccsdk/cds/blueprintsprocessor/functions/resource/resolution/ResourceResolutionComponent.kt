...
import org.onap.ccsdk.cds.blueprintsprocessor.services.execution.error.data.CDSErrorImpl
...
open class ResourceResolutionComponent(private val resourceResolutionService: ResourceResolutionService) :
    ...
    override suspend fun processNB(executionRequest: ExecutionServiceInput) {
        ...
        if (storeResult) {
            val errorType = BlueprintProcessorErrorCodes.GENERIC_FAILURE
            val cdsErrorImpl = CDSErrorImpl(errorType.httpCode, errorType.name)
            if (resolutionKey.isNotEmpty() && (resourceId.isNotEmpty() || resourceType.isNotEmpty())) {
                val cdsErrorException = CDSErrorException(cdsErrorImpl, message = "Can't proceed with the " +
                        "resolution: either provide resolution-key OR combination of resource-id and resource-type.")
                cdsErrorException.addErrorModel(errorType)
                throw cdsErrorException
            } else if ((resourceType.isNotEmpty() && resourceId.isEmpty()) || (resourceType.isEmpty() && resourceId.isNotEmpty())) {
                val cdsErrorException = CDSErrorException(cdsErrorImpl, message = "Can't proceed with the resolution: both resource-id and resource-type" +
                        " should be provided, one of them is missing.")
                cdsErrorException.addErrorModel(errorType)

                throw cdsErrorException
            } else if (resourceType.isEmpty() && resourceId.isEmpty() && resolutionKey.isEmpty()) {
                val cdsErrorException = CDSErrorException(cdsErrorImpl, message = "Can't proceed with the resolution: can't persist " +
