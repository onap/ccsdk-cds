...
open class BluePrintRestLibPropertyService(private var bluePrintProperties:
    ...
    private fun restClientProperties(jsonNode: JsonNode): RestClientProperties {
        ...
        return when (type) {
            ...
            RestLibConstants.TYPE_SSL_BASIC_AUTH -> {
                getSslBasicAuthProps(jsonNode)
            }
            RestLibConstants.TYPE_SSL_TOKEN_AUTH -> {
                getSslTokenAuthProps(jsonNode)
            }
            RestLibConstants.TYPE_SSL_NO_AUTH -> {
    ...
    private fun blueprintWebClientService(
            restClientProperties: RestClientProperties): BlueprintWebClientService {
    ...
    private fun getSslBasicAuthProps(jsonNode: JsonNode): RestClientProperties {

        val rest: SSLBasicAuthRestClientProperties  = JacksonUtils.readValue(
                jsonNode, SSLBasicAuthRestClientProperties::class.java)!!
        val basic : BasicAuthRestClientProperties = JacksonUtils.readValue(
                jsonNode, BasicAuthRestClientProperties::class.java)!!
        rest.basicAuth= basic
        return rest
    }

    private fun getSslTokenAuthProps(jsonNode: JsonNode): RestClientProperties {

        val rest: SSLTokenAuthRestClientProperties  = JacksonUtils.readValue(
                jsonNode, SSLTokenAuthRestClientProperties::class.java)!!
        val token : TokenAuthRestClientProperties = JacksonUtils.readValue(
                jsonNode, TokenAuthRestClientProperties::class.java)!!
        rest.tokenAuth= token
        return rest
    }
