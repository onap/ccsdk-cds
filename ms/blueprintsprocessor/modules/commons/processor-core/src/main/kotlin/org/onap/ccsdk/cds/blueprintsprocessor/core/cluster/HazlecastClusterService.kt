            ...
            when (configuration) {
                ...
                is ClusterInfo -> {
                    if (configuration.configFile != null) {
                        ...
                        if (configFile.endsWith("xml", true)) {
                            FileSystemXmlConfig(normalizedFile(configFile))
                        } else if (configFile.endsWith("yaml", true)) {
                            FileSystemYamlConfig(normalizedFile(configFile))
                        } else {
                            throw BluePrintProcessorException(
                                "could understand cluster config " +
                                    "file(${configuration.configFile}) format, it should be xml or yaml"
                            )
                        }
                    } else {
                        val config = Config()
