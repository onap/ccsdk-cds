...
class BlueprintSshClientServiceTest {
    ...
    fun testBasicAuthSshClientService() {
        runBlocking {
            ...
            val response = bluePrintSshLibPropertyService.executeCommands(arrayListOf("echo '1'", "echo '2'"), 2000)
    ...
    @Test
    fun `testBasicAuthSshClientService running commands in different contexts`() {
        runBlocking {
            val sshServer = setupTestServer("localhost", 52815, "root", "dummyps")
            sshServer.start()
            println(sshServer)
