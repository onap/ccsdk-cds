...
open class BasicAuthSshClientService(private val basicAuthSshClientProperties: BasicAuthSshClientProperties) :
    ...
    private val newLine = "\n".toByteArray()
    //private val OUTPUT_REGEX = ".*[\\r\\n]*(?<output>[\\s\\S]*?)[\\r\\n]*(?<prompt>.*#)$" //TODO for each device filter output
    //private val PROMPT_PATTERN = Pattern.compile(OUTPUT_REGEX) //TODO for each device filter output
    //private val OUTPUT_GROUP_NAME = "output"  //TODO for each device filter output
    ...
    override suspend fun executeCommandNB(command: String, timeOut: Long): CommandResult {
        ...
        log.info("Command Response: ({})", commandResult)
    ...
    private fun waitForPrompt(timeOut: Long): String {
        ...
        if (channel!!.out.toString().indexOf("$") <= 0 && waitMask.contains(ClientChannelEvent.TIMEOUT)) {
    ...
    //TODO filter output for each type of ssh client device
