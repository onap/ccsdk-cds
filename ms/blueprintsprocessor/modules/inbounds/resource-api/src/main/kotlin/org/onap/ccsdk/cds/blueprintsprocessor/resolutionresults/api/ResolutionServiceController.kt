...
open class ResolutionServiceController(private var resourceResolutionDBService: ResourceResolutionDBService) {
    ...
    fun getFromKey(@RequestParam(value = "bpName") bpName: String,
        ...
        if (name.isNotEmpty()) {
            ResponseEntity.ok().body(listOf(resourceResolutionDBService.readValue(bpName, bpVersion, artifactName, resolutionKey, name)))
        } else {
            ResponseEntity.ok().body(resourceResolutionDBService.readArtifact(bpName, bpVersion, artifactName, resolutionKey))
        }
    }
    ...
    fun getFromResourceId(@RequestParam(value = "bpName") bpName: String,
        ...
        ResponseEntity.ok().body(resourceResolutionDBService.readWithResourceId(bpName, bpVersion, resourceId, resourceType))
    }
}
