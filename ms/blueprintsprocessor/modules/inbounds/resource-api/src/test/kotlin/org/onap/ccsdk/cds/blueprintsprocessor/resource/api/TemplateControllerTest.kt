...
class TemplateControllerTest {
    ...
    fun `store same value and tries to retrieve - duplicate entry execption`() {
        runBlocking {
            ...
            webTestClient
                .post()
                .uri("/api/v1/template/$blueprintName/$blueprintVersion/$templatePrefix/$resolutionKey")
    ...
    fun `get returns requested XML content-type`() {
        runBlocking {
            webTestClient
                .post()
                .uri("/api/v1/template/$blueprintName/$blueprintVersion/$templatePrefix/${resolutionKey}1")
                .body(BodyInserters.fromObject(payloadDummyTemplateData))
                .exchange()
                .expectStatus().is2xxSuccessful
                .expectBody()
            ...
            requestArguments = "bpName=$blueprintName&bpVersion=$blueprintVersion" +
                    "&artifactName=$templatePrefix&resolutionKey=${resolutionKey}1&format=xml"
            webTestClient
                .get()
                .uri("/api/v1/template?$requestArguments")
                .exchange()
                .expectStatus().is2xxSuccessful
                .expectHeader().contentType(MediaType.valueOf("application/xml"))
                .expectBody().equals(payloadDummyTemplateData)
        }
    }
