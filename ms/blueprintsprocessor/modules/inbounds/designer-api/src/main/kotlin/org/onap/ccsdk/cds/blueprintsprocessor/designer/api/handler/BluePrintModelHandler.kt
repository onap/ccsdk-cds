...
import org.onap.ccsdk.cds.blueprintsprocessor.designer.api.WorkflowModelRequest
import org.onap.ccsdk.cds.blueprintsprocessor.designer.api.WorkflowModelResponse
...
open class BluePrintModelHandler(
    ...
    open suspend fun prepareWorkFlowModel(req: WorkflowModelRequest):
            ResponseEntity<WorkflowModelResponse> {
        ...
        val workflowModelResponse = WorkflowModelResponse()
        workflowModelResponse.blueprintName = req.blueprintName
        workflowModelResponse.version = req.version
        ...
        for ((k, v) in workFlow.inputs!!) {
            var data = blueprintContext.dataTypeByName(v.type)
            if (data != null) {
                workflowModelResponse.dataTypes?.put(v.type, data)
            }
        }
        ...
        workflowModelResponse.workFlowData = workFlowData
        return ResponseEntity(workflowModelResponse, HttpStatus.OK)
    }

    @Throws(BluePrintException::class)
    open suspend fun getWorkflows(name: String, version: String):
            ResponseEntity<WorkflowsResponse> {
        val basePath = blueprintsProcessorCatalogService.getFromDatabase(name, version)
        log.info("blueprint base path $basePath")
