...
class KafkaPublishAuditService(
     ...
     * The correlation ID is used to link the input to its output.
     * Sensitive data within the request are hidden.
     */
    override suspend fun publish(correlationId: String, executionServiceInput: ExecutionServiceInput, blueprintRuntimeService: BluePrintRuntimeService<*>?) {
        executionServiceInput.commonHeader.correlationId = correlationId
     ...
     * The correlation ID is used to link the output to its input.
     * A correlation ID is added to link the input to its output.
     */
    override fun publish(correlationId: String, executionServiceOutput: ExecutionServiceOutput) {
        executionServiceOutput.commonHeader.correlationId = correlationId
