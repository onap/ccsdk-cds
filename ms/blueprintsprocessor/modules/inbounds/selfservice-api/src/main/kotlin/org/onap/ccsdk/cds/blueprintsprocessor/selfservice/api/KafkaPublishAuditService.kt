...
class KafkaPublishAuditService(
    ...
    private suspend fun hideSensitiveData(
        ...
        val blueprintName = executionServiceInput.actionIdentifiers.blueprintName
        if (blueprintName == "default") return executionServiceInput

        val requestId = executionServiceInput.commonHeader.requestId
        val blueprintVersion = executionServiceInput.actionIdentifiers.blueprintVersion
        ...
        val sensitiveParameters: List<String> = artifactPrefixNames
                ...
                .filter { "input-source".equals(it.dictionaryName) && PropertyDefinitionUtils.hasLogProtect(it.property) }
                .map { it.name }
