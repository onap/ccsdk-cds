...
class CDSRequestLockingManager {
    ...
    fun canExecutionStartOrEnqueued(serviceInput: ExecutionServiceInput): EnqueueStatus {
        ...
        val queueByKey = queues.getOrPut(qKey) {
            log.debug("Creating a new queue for resourceModelId/Type ($resId/$resType)")
        ...
        log.debug("Adding requestId (${serviceInput.commonHeader.requestId}) with resourceModelId/Type ($resId/$resType) to the queue. Queue execution status ($enqueueStatus). #elements in the queue ${queueByKey.size}")
    ...
    fun remove(execServiceInput: ExecutionServiceInput) {
        ...
        val qKey = ResourceModelIdType(commonHeader.resourceModelId!!, commonHeader.resourceModelType!!) // composite key
        log.debug("Removing requestId ($reqId) for $qKey")
        queues[qKey]?.let {
