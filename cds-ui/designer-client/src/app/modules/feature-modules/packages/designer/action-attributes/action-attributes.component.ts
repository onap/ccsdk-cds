...
export class ActionAttributesComponent implements OnInit {
    ...
    suggestedOutputsAndAttributes: string[] = [];
    ...
    getAttributesAndOutputs(functionName: string) {
        this.suggestedOutputsAndAttributes = [];
            ...
            .forEach(currentFunction => {
                if (currentFunction.definition['attributes']) {
                    Object.keys(currentFunction.definition['attributes']).forEach(attribute => {
                        this.suggestedOutputsAndAttributes.push(attribute);
                    });
                }
                if (currentFunction['definition']['interfaces']
                    [Object.keys(currentFunction['definition'] && currentFunction['definition']['interfaces'])]
                    ['operations']['process']['outputs']) {
                    const interfaceName = Object.keys(currentFunction['definition']['interfaces'])[0];
                    Object.keys(
                        currentFunction['definition']['interfaces'][interfaceName]
                            ['operations']['process']['outputs'])
                        .forEach(attribute => {
                            this.suggestedOutputsAndAttributes.push(attribute);
                        });
                ...
                console.log(this.suggestedOutputsAndAttributes);
