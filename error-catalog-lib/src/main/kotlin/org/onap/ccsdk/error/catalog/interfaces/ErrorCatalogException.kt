...
abstract class ErrorCatalogException : RuntimeException {
    ...
    constructor(errorCatalog: ErrorCatalog, logLevel: String = LogLevel.ERROR.name, message: String, cause: Throwable):
            super(message, cause) {
        this.errorPayload = ErrorPayload(errorCatalog.code, errorCatalog.errorId, logLevel, cause.message ?: message)
        addErrorModel(ErrorMessage(errorCatalog.domainId, message, cause))
    }

    constructor(errorCatalog: ErrorCatalog, logLevel: String = LogLevel.ERROR.name, message: String, cause: Throwable, vararg args: Any?) :
