...
import org.onap.ccsdk.error.catalog.repository.ErrorMessageRepository
...
open class ErrorCatalogDatabaseHandler {
     ...
     *
     * @return MutableMap<String, ErrorMessage> list of the errorMessage
    </Domain> */
    open fun getAllErrorMessagesByApplication(applicationId: String): MutableMap<String, ErrorMessage> {
        val domains = domainRepository.findAllByApplicationId(applicationId)
        val errorMessages = mutableMapOf<String, ErrorMessage>()
        for (domain in domains){
            val errorMessagesFound = errorMessageRepository.findByDomainsId(domain.id)
    ...
    open fun saveDomain(domain: String, applicationId: String, description: String = "", errorMessageList: List<ErrorMessage>) {
