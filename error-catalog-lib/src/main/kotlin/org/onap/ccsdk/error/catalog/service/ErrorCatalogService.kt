...
class ErrorCatalogService(private var errorMessagesLibService: ErrorMessagesLibService) {
    ...
    fun getErrorCatalog(
        ...
        val message = getMessage(enumErrorCatalog.getErrorDomain(), enumErrorCatalog.getErrorName()) ?: message
        val code = code ?: getProtocolErrorCode(enumErrorCatalog, protocol)
        val action = ErrorCatalogUtils.readErrorActionFromMessage(message)
        val cause = cause.message ?: ErrorCatalogUtils.readErrorCauseFromMessage(message) ?: message
